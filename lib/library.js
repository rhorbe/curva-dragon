      ...
      while (distance > 0) {
        ...
        if (turtle.wrap) {
          if (insideCanvas(newX, newY, minX, maxX, minY, maxY)) {
            ...
            (point = intersect(x, y, newX, newY, maxX, maxY, maxX, minY))
          )
            xWrap(maxX, minX);
          else if (
            (point = intersect(x, y, newX, newY, minX, maxY, minX, minY))
          )
            xWrap(minX, maxX);
          else if (
            (point = intersect(x, y, newX, newY, minX, maxY, maxX, maxY))
          )
            yWrap(maxY, minY);
          else if (
            (point = intersect(x, y, newX, newY, minX, minY, maxX, minY))
          )
            yWrap(minY, maxY);
...
/*   canv.addEventListener("mousemove", function (event) {
  ...
  function repeat(n, action) {
    for (var count = 1; count <= n; count++) action();
  }
